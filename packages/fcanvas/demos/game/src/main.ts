...
class Sprite {
  ...
  display(camera) {
    ...
    const frame = this.anim.frameset[this.anim.frame]
    ...
    ctx.drawImage(
      this.image,
      (frame % this.data.width) * this.data.tileWidth,
      Math.floor(frame / this.data.width) * this.data.tileHeight,
      this.data.tileWidth,
...
class Player extends Mover {
  ...
  update(dt) {
    const bottom = this.updateMovement(dt)[3]

    this.handleAnim(bottom)

    if (
      (this.pos.x + this.p2[0] - finish[0]) *
        (this.pos.x + this.p1[0] - finish[0]) <
        0 &&
      (this.pos.y + this.p2[1] - finish[1]) *
        (this.pos.y + this.p1[1] - finish[1]) <
        0
    ) {
...
class Fireball extends Mover {
  ...
  update(dt) {
    ...
    if (
      (collide[0] || collide[1] || this.collide(player)) &&
      this.getAnim() == "fireball"
    ) {
      this.state[0] = 0
...
function preload() {
  ...
  loader
    .load({
      tileset:
        "https://raw.githubusercontent.com/dada542/just-platformer-game/main/tiles.png",

      player:
        "https://raw.githubusercontent.com/dada542/just-platformer-game/main/player.png",

      enemies:
        "https://raw.githubusercontent.com/dada542/just-platformer-game/main/enemies.png",

      items:
        "https://raw.githubusercontent.com/dada542/just-platformer-game/main/items.png",

      tilemap:
        "https://raw.githubusercontent.com/dada542/just-platformer-game/main/mapa4.json"
...
function getLikes() {
  ...
  xhttp.open(
    "GET",
    "https://cors-anywhere.herokuapp.com/https://code.sololearn.com/WuTAHIogb4b2/",
    true
  )
  ...
  return new Promise((resolve) => {
    ...
    const likes = parser
      ...
      .querySelector(".positive").innerHTML

    document.getElementById("likes").innerHTML =
      "Likes for 2nd part: " + likes.slice(1) + "/200"
...
function main() {
  ...
  button1.addEventListener(
    "ontouchstart" in document ? "touchend" : "mouseup",
    (e) => {
      player.state[0] = 0
    }
  )
