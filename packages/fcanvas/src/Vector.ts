...
function calculateRemainder2D(
  ...
  if (xComponent !== 0) this.x %= xComponent

  if (yComponent !== 0) this.y %= yComponent
...
function calculateRemainder3D(
  ...
  if (zComponent !== 0) this.z %= zComponent
...
export class Vector {
  ...
  div(n: number): this {
    if (n === 0) {
      if (__DEV__) console.warn("div:", "divide by 0")
      return this
    }
  ...
  lerp(x: Vector | number = 0, y = 0, z = 0, amt = 0): this {
    if (x instanceof Vector) return this.lerp(x.x, x.y, x.z, y)
