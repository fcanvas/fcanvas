...
export function createFilter(options: OptionFilter): string {
      ...
      let filter : string = ""
        ...
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          filter += `drop-shadow(${options!.dropShadow.x ?? 0}px ${options!.dropShadow.y ?? 0}px ${options!.dropShadow.blur ?? 0}px ${options!.dropShadow.color})`
        }
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
